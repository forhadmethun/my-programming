...
# file = open('/home/f/Code/Others/general-topics/cyber-security/applied_crypto/lab5/priv.pem','r')
# content = file.read()
# file = open('/home/f/Code/Others/general-topics/cyber-security/applied_crypto/lab5/priv.der','wb')
# file.write(pem_to_der(content))
# file.close()


# todo file der & pem!! how to handle
def get_pubkey(filename):
    ...
    # keyfile = pem_to_der(keyfile.read())
    # keyfile = codecs.decode(keyfile.read(), 'base64')
    der = pem_to_der(keyfile.read())
    bitstring = str(decoder.decode(der)[0][1])
    # convert BITSTRING to bytestring
    # todo : improve if possible later
    ...
    # nb(int(pubkey[0]), len(str(pubkey[0])))
...
def get_privkey(filename):
    ...
    # bytestring = bytes(int(bitstring[i: i + 8], 2) for i in range(0, len(bitstring), 8))
    # decode the bytestring (which is actually DER) and return (n, e)
    # privkey[0] = decoder.decode(der)[0][1]
    # privkey[1] = decoder.decode(der)[0][3]

    return int(privkey[0][1]), int(privkey[0][3])


# get_privkey('/home/f/Code/Others/general-topics/cyber-security/applied_crypto/lab5/priv.der')
...
# pkcsv15pad_encrypt('12345',1029035719873479896203996843978945819540390654141)
...
def pkcsv15pad_sign(plaintext, n):
    ...
    padding_string = b''
    while padding_string_length > 0:
        padding_string += bytes([0x00])
        padding_string_length -= 1
    padded_plaintext = bytes([0x00, 0x01]) + padding_string + bytes([0xff]) + plaintext  # .encode()
    return padded_plaintext
...
# encrypt('pub.pem', 'plain.txt','enc.txt')
...
# decrypt('priv.pem', 'enc.txt', 'mydec.txt')
# decrypt('priv.key', 'enc.txt', 'mydec.txt')
...
verify('pub.pem', 'signature', 'filetosign')
# def usage():
#     print
#     "Usage:"
#     print
