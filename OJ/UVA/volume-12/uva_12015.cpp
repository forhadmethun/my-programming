...
int main(int argc, char const *argv[])
{
    ...
    {
        printf("Case #%d:\n",t+1);
        char in[120],str[120];int c;
        msi mi,*it;pair<string,int> arr[12];
        F(i,10){
            //getline(ci)
            gets(in);
            //cout << in << endl;
            sscanf(in,"%s%*c%d",str,&c);
            //cout << "str: " << str << "ssssssssss " << c  << endl;
            //it = mi.find(str);
        ...
        F(j,10)
        {
            if(arr[j].second > mx) mx = arr[j].second;
        }
        F(j,10)
        {
            if(arr[j].second ==  mx) //mx = arr[j].second;
        ...
        {
            //cout << (string) it->first << " times: " << it->second   << endl;
            if( (it->second) > mx)
            {
                mx  = it->second;
            }
