...
#define TT 256
//int *arr =(int) calloc(sizeof(int) * TT);
int arr[TT];
...
int main(int argc, char const *argv[])
{
    ...
    while(n--){
        ...
        FOR(i,s.size()){
         ...
         //cout << " Mx: " << mx  << "in: " << index << endl;

         //FOR(i,s.size())if(arr[s[i]] > mx) mx = arr[s[i]];


         FOR(i,TT)if(arr[i] == mx) printf("%c",i);
