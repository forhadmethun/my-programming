...
# took 18.00 hours (please specify here how much time your solution required)
...
def get_name(cert):
    ...
    name = (encoder.encode(decoder.decode(cert)[0][0][3]))
    return name

def get_key(cert):
     # get subjectPublicKey from certificate
    subjectPublicKey =(nb(bin_string_to_int(str(decoder.decode(cert)[0][0][6][1]))))
    return subjectPublicKey

def get_serial(cert):
    # get serial from certificate
    serial = asn1_integer((decoder.decode(cert)[0][0][1]))
    return serial
                    ...
                    asn1_sequence(
                        ...
                        asn1_sequence(
                            ...
                            asn1_sequence(
                                ...
                                +
                                #issuer name
                                asn1_octetstring(hashlib.sha1(get_name(cert)).digest())  # .hexdigest()
                                #issuer key hash
                                +
                                asn1_octetstring(hashlib.sha1(get_key(issuer_cert)).digest())
                                #serial number
                                +
                                get_serial(cert)
...
def send_req(ocsp_req, ocsp_url):
    ...
    payload = header_bytes.encode() + (body_bytes)
    s.send(payload)
