...
for i in range(1, number_of_fields + 1):
    send([0x00, 0xA4, 0x02, 0x0C] + [0x02, 0x50, i])  # MF/5000/5006
...
send([0x00, 0xA4, 0x00, 0x0C]) # SELECT FILE (MF)
# r = send([0x00, 20, 0x00, 0x01, 0x00])  # READ BINARY
pin1 = channel.transmit([0x00, 0x20, 0x00, 0x01, 0x02])
puk = channel.transmit([0x00, 0x20, 0x00, 0x02, 0x02])
send([0x00, 0xA4, 0x00, 0x0C]) # SELECT FILE (MF)
send([0x00, 0xA4, 0x01, 0x0C]+[0x02, 0xad, 0xf2]) # MF/5000
pin2 = channel.transmit([0x00, 0x20, 0x00, 0x85, 0x02])
print("\t" + "PIN1: " + hex(pin1[2])[-1] + " left")
print("\t" + "PIN2: " + hex(pin2[2])[-1] + " left")
print("\t" + "PUK: " + hex(puk[2])[-1] + " left")
