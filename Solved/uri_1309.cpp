...
int main(int argc, char const *argv[])
{
    ...
    string amount;
    while(getline(cin,amount)!=NULL)
    {
        string precision;
        getline(cin,precision);
        cout << "$";
        int mod = amount.size() % 3 -1 ;
        FOR(i,amount.size())
        {
            cout << amount[i];
            if((i-mod)%3 == 0 && i!=(amount.size()-1))
            {
                cout << ",";
            }
        ...
        cout << "." ;
        if(precision.length()==2)cout << precision <<endl;
        else if(precision.length() ==1)cout << "0" << precision <<endl;
        else cout << "00"<<endl ;
